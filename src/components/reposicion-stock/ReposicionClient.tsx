...
import { listReposiciones, createReposicion, type ReposicionStock } from '../../lib/api';
import { formatCurrency } from '../../lib/data';
import ReposicionModal from './ReposicionModal';

export default function ReposicionClient() {
  const [items, setItems] = useState<ReposicionStock[]>([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');
  const [modalOpen, setModalOpen] = useState(false);
  const [editItem, setEditItem] = useState<ReposicionStock | null>(null);
  ...
  const handleEdit = (item: ReposicionStock) => { setEditItem(item); setModalOpen(true); };
  ...
  return (
    <div>
      <div className="flex justify-between items-end mb-8">
        <div>
          ...
          <p className="font-body-base text-on-surface-variant">
            Fondos destinados a la reposición de inventario.
          </p>
        </div>
        <button
      ...
      <ReposicionModal open={modalOpen} onClose={() => { setModalOpen(false); setEditItem(null); }} editItem={editItem} onSaved={handleSaved} />
    </div>
  );
}
