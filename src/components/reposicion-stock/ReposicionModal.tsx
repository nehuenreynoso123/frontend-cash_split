...
import type { ReposicionStock } from '../../lib/api';
import { createReposicion } from '../../lib/api';

interface Props {
  open: boolean;
  onClose: () => void;
  editItem: ReposicionStock | null;
  onSaved: () => void;
}

export default function ReposicionModal({ open, onClose, editItem, onSaved }: Props) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setSaving(true);
    try {
      await createReposicion({ nombre, descripcion, monto: parseFloat(monto) });
      onSaved();
    } catch (err) {
    ...
    <Modal open={open} onClose={onClose} title="Nuevo Movimiento de Stock">
      <form onSubmit={handleSubmit} className="p-8 space-y-6">
        ...
        <div>
          ...
          <input className="w-full px-4 py-2.5 border border-outline-variant rounded-lg focus:ring-2 focus:ring-secondary/20 focus:border-secondary outline-none" placeholder="Ej: Reposición Q1" value={nombre} onChange={(e) => setNombre(e.target.value)} required />
        </div>
        <div>
          <label className="block font-label-caps text-label-caps text-on-surface-variant uppercase mb-2">Descripción</label>
          <input className="w-full px-4 py-2.5 border border-outline-variant rounded-lg focus:ring-2 focus:ring-secondary/20 focus:border-secondary outline-none" placeholder="Ej: Compra de nuevos equipos" value={descripcion} onChange={(e) => setDescripcion(e.target.value)} required />
        </div>
        <div>
